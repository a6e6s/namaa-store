...
<div class="right_col" role="main">
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <form action="<?php echo ADMINURL . '/donations/edit/' . $data['donation']->donation_id; ?>" method="post" enctype="multipart/form-data" accept-charset="utf-8">
                <div class="col-sm-12 col-xs-12">
                    <div class="form-group">
                        <label class="control-label" for="pageTitle">معرف التبرع : </label>
                        <div class="has-feedback">
                            <input type="text" class="form-control" name="donation_identifier" readonly value="<?php echo $data['donation']->donation_identifier; ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="amount">قيمة التبرع : </label>
                        <div class="has-feedback">
                            <input type="number" class="form-control" name="amount" required value="<?php echo $data['donation']->amount; ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="quantity"> الكمية : </label>
                        <div class="has-feedback">
                            <input type="number" class="form-control" name="quantity" required value="<?php echo $data['donation']->quantity; ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="total"> الاجمالي : </label>
                        <div class="has-feedback">
                            <input type="number" class="form-control" name="total" required value="<?php echo $data['donation']->total; ?>">
                        </div>
                    </div>
                        ...
                        <div class="has-feedback">
                            <select name="project_id" class="form-control">
                                ...
                                <?php foreach ($data['projectList'] as $project) : ?>
                                    <option value="<?php echo $project->project_id; ?>" <?php echo ($project->project_id == $data['donation']->project_id) ? " selected " : ''; ?>>
                        ...
                        <div class="has-feedback">
                            <select name="payment_method_id" class="form-control">
                                ...
                                <?php foreach ($data['paymentMethodsList'] as $payment_method) : ?>
                                    <option value="<?php echo $payment_method->payment_id; ?>" <?php echo ($payment_method->payment_id == $data['donation']->payment_method_id) ? " selected " : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="status_id"  data-placeholder="اختار الوسوم المناسبة" style="width: 100%;">
                            <?php foreach ($data['statusesList'] as $status) : ?>
                                <option value="<?php echo $status->status_id; ?>" <?php echo ($status->status_id==  $data['donation']->status_id) ? " selected " : 'no'; ?>>
                        ...
                        <div class="form-group <?php echo (empty($data['banktransferproof_error'])) ?: 'has-error'; ?>">
                            ...
                            <div class="has-feedback input-group">
                                <span class="input-group-btn">
                                    ...
                                    <input name="banktransferproof" value="<?php echo ($data['donation']->banktransferproof); ?>" onchange="$(this).parent().parent().find('.form-control').html($(this).val().split(/[\\|/]/).pop());" style="display: none;" type="file">
                                </span>
                                <span class="form-control"><small><?php echo empty($data['donation']->banktransferproof) ? 'قم بأختيار صورة مناسبة' : $data['donation']->banktransferproof; ?></small></span>
                            </div>
                    ...
                    <div class="form-group col-xs-12 <?php echo (!empty($data['status_error'])) ? 'has-error' : ''; ?>">
                        ...
                        <div class="radio">
                            <label>
                                <input type="radio" class="flat" <?php echo ($data['donation']->status == 1) ? 'checked' : ''; ?> value="1" name="status"> مؤكد
                            </label>
                            <label>
                                <input type="radio" class="flat" <?php echo ($data['donation']->status == 0) ? 'checked' : ''; ?> value="0" name="status"> غير مؤكد
                            </label>
                            <label>
                                <input type="radio" class="flat" <?php echo ($data['donation']->status == 4) ? 'checked' : ''; ?> value="4" name="status"> ملغاه
                            </label>
                            <label>
                                <input type="radio" class="flat" <?php echo ($data['donation']->status == 3) ? 'checked' : ''; ?> value="3" name="status"> في الانتظار
                            </label>
